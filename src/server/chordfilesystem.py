...
class FileSystemNode(ChordNode):
    ...
    def _request_with_permission(self, tags, files_names, query_tags, callback):
        """
        Solicita permiso al líder antes de realizar una operación.

        Parámetros:
        tags (list): Lista de etiquetas.
        files_names (list): Lista de nombres de archivos.
        query_tags (list): Lista de etiquetas de consulta.
        callback (función): Función que se ejecutará si se concede el permiso.

        Retorna:
        bool: True si se obtiene permiso, False en caso contrario.
        """
        in_election = False if self.election.leader else True
        print(f"***************{in_election}**********")
        # Verificar la existencia del líder
        if not in_election:
            leader_ref = ChordNodeObject(self.election.leader)
            if not leader_ref.check_node():
                self.election.leader_lost()
                in_election = True
        print("***************DESPUES **********")
        # Esperar hasta 10 segundos la elección del líder
        wait_time = 10
        while in_election:
            if wait_time == 0:
                print("Solicitud de espera para continuar con la elección del líder")
