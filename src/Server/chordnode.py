...
class ChordNode:
    def __init__(self, context : ssl.SSLContext, ip: str, m: int = 160, update_replication=None):
        """
                        ...
                        if x and inbetween(x.id, self.id, self.succ.id):
                            if x.id != self.succ.id:
                                ...
                                if self.update_replication:
                                    self.update_replication(True, False, False, False)
            ...
            try:
                if self.pred and not self.pred.check_node():
                    ...
                    if self.pred.id == self.id:
                        ...
                        self.predpred = None
                        if self.update_replication:
                                self.update_replication(False, False, True, True, assume_predpred=self.ip)
