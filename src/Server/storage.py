...
class Storage:
    ...
    def assume_data(self, successor_ip: str, new_predecessor_ip: str = None, assume_predpred: str = None):
        ...
        if assume_predpred:
            print(f"Asumiendo datos del prepredecesor....")
            # Asumir etiquetas replicadas
            self.tags.update(self.replicated_pred2_tags)
            print(f"{len(self.replicated_pred2_tags.items())} etiquetas asumidas del prepredecesor")
            self.save_tags()
            ...
            # Asumir archivos replicados
            self.files.update(self.replicated_pred2_files)
            print(f"{len(self.replicated_pred2_files.items())} archivos asumidos del predecesor")
            self.save_files()
    ...
    def delegate_data(self, new_owner_ip: str, successor_ip: str, predecessor_ip: str, case_2: bool):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            s.sendall(f"{self.storage_ip};{'1' if case_2 else '0'}".encode('utf-8'))
            s.close()
    ...
    def pull_replication(self, owner_ip: str, is_pred: bool = True):
        ...
        if is_pred:
            ...
            for k, _ in self.replicated_pred_files.items():
                if os.path.exists(f"{self.replicated_pred_bins_path}/{k}"):
            ...
            # Eliminar las réplicas actuales
            for k, _ in self.replicated_pred2_files.items():
                if os.path.exists(f"{self.replicated_pred2_bins_path}/{k}"):
    ...
    def _handle_recv(self, conn: socket.socket, data: str):
        ...
        if data == f"{REPLICATE_PRED_STORE_TAG}":
            ...
            conn.sendall(f"{OK}".encode('utf-8'))
            file_name = conn.recv(1024).decode('utf-8')
            if os.path.exists(f"{self.replicated_pred_bins_path}/{file_name}"):
